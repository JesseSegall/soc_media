...
export default function ProfileContent({ activeTab, userId }) {
	...
	// Grabs all posts and saves inside state
	useEffect(() => {
		if (!userId) {
			return;
		}
	...
	// grabs posts from the two async functions below
	...
	return (
		<div>
			{activeTab === 'posts' && (
				<div>
					{posts?.length > 0 &&
						posts.map((post) => <PostCard key={post.created_at} {...post} profiles={profile} />)}
				</div>
			)}
			{activeTab === 'about' && (
				<div>
